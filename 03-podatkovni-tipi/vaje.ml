...
type dollar = Dolar of float

let dollar_to_euro (Dolar d) = Euro (d *. 1.13)

let euro_to_dollar (Euro e) = Dolar (e *. 0.86)
...
let rec intbool_reverse l = 
  let rec intbool_reverse' acc =
    function
    | Nil -> acc
    | Int (i, l') -> intbool_reverse' (Int (i, acc)) l'
    | Bool (b, l') -> intbool_reverse' (Bool (b, acc)) l'
in
intbool_reverse' Nil l
